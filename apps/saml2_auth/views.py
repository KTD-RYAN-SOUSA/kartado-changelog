#!/usr/bin/env python
# -*- coding:utf-8 -*-
# copied this entire app from https://github.com/fangli/django-saml2-auth
# decided to copy instead of fork/PR because the repo has 14 open PRs
# and it seems like the original writer doesn't care enough to merge them
...
def jwt_encode(user):
    ...
    return jwt_encode_handler(payload, db_alias="default")
...
def get_reverse(objs):
    ...
    for obj in objs:
        try:
            ...
            pass
    raise Exception(
        "We got a URL reverse issue: %s. This is a known issue but \
        please still submit a ticket at https://github.com/fangli/django-saml2-auth/issues/new"
        % str(objs)
    )
...
def denied(r):
    frontend_url = settings.SAML2_AUTH.get("FRONTEND_URL")
    denied_message = settings.SAML2_AUTH.get("DENIED_MESSAGE")
    return HttpResponseRedirect("{}?message={}".format(frontend_url, denied_message))
...
def acs(r):
    ...
    try:
        target_user = User.objects.get(
            saml_nameid=user_name, saml_idp=authn_response.issuer()
        )
    ...
    if settings.SAML2_AUTH.get("USE_JWT") is True:
        ...
        query = "?uid={}&token={}&roadlabs=true".format(target_user.uuid, jwt_token)

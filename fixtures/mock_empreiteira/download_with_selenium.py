...
# If there are multiple keywords, each main_keyword will join with each
# supplemented_keyword use selenium and urllib, and each search query will download any
# number of images that google provide allow single process or multiple processes for
# downloading.
# Pay attention: since selenium is used, geckodriver and firefox browser is required.
########################################################################################
...
def get_image_links(
    main_keyword, supplemented_keywords, link_file_path, num_requested=10
):
    ...
    for i in range(len(supplemented_keywords)):
        ...
        print(
            "Process-{0} add keyword {1} , got {2} image urls so far".format(
        ...
        for link in rf:
            try:
                ...
                with open(file_path, "wb") as wf:
                    wf.write(data)
                print(
                    "Process-{0} download image {1}/{2}.jpg".format(
                        main_keyword, main_keyword, count
                    )
                )
                count += 1
                if count % 10 == 0:
                ...
                print("URLError")
                logging.error(
                    "URLError while downloading image {0}reason:{1}".format(
                        link, e.reason
                    )
                )
                continue
                ...
                logging.error(
                    (
                        "HTTPError while downloading image "
                        "{0}http code {1}, reason:{2}"
                    ).format(link, e.code, e.reason)
                )
                continue
                ...
                logging.error(
                    (
                        "Unexpeted error while downloading image "
                        "{0}error type:{1}, args:{2}"
                    ).format(link, type(e), e.args)
                )
                continue


def do_stuff(main_keywords, supplemented_keywords, download_dir, limit):
    # download_dir = './data/'
    ...
    # multiple processes
    # default number of process is the number of cores of your CPU,

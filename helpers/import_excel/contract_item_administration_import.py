...
from apps.templates.models import ExcelContractItemAdministration, ExcelImport
from apps.templates.serializers import ExcelContractItemAdministrationSerializer
...
class ImportContractItemAdministration:
    """
    Class responsible for handling the import of ContractItemAdministration objects from Excel
    """

    temp_path = "/tmp/contract_item_administration_excel_import/"

    REQUIRED_FIELDS = [
        "secao de administracao",
        "codigo",
        "recurso",
        "quadro de controle",
        "quantidade",
    ...
    COLUMN_NAME_MAPPING = {
        "section_name": "Seção de Administração",
        "sort_string": "Código",
    ...
    def __init__(self, excel_import, user):
        ...
        self.file_name = ""
        self.created_contract_items_administration = []
        self.preview_contract_items_administration = []
        self.errors = []
    ...
    def process_items_sheet(self):
        try:
            contract_items_administration_sheet, header_map = self.find_items_sheet()
            if not contract_items_administration_sheet or not header_map:
                return False

            section_idx = header_map.get("secao de administracao")
            ...
            control_board_idx = header_map.get("quadro de controle")
            ...
            data_rows = list(contract_items_administration_sheet.values)[1:]
            data_rows_count = 0
            for row_idx, row in enumerate(data_rows, 2):
                ...
                control_board = (
                    row[control_board_idx]
                    if control_board_idx is not None and control_board_idx < len(row)
                    else None
                )
                ...
                item_dict["control_board"] = control_board
                ...
                if not control_board:
                    column_errors.append("control_board")
                ...
                if section_name:
                    section = ContractService.objects.filter(
                        ...
                        administration_service_contracts__uuid=self.contract_id,
                    ).first()
                else:
                    section = None
                ...
                if not section:
                    item_dict["section_error"] = {
                        "error": "Seção de administração não encontrada"
                    }
                ...
                content_type = None
                if control_board:
                    control_board_clean = clean_latin_string(
                        str(control_board).lower().strip()
                    )

                    control_board_mapping = {
                        "funcionario": ("daily_reports", "dailyreportworker"),
                        "equipamento": ("daily_reports", "dailyreportequipment"),
                        "veiculo": ("daily_reports", "dailyreportvehicle"),
                    }

                    if control_board_clean.endswith("s"):
                        control_board_clean = control_board_clean[:-1]

                    if control_board_clean in control_board_mapping:
                        app_label, model = control_board_mapping[control_board_clean]
                        content_type = ContentType.objects.get(
                            app_label=app_label, model=model
                        )
                    else:
                        item_dict["control_board_error"] = {
                            "error": "Quadro de controle deve ser 'Funcionário', 'Equipamento' ou 'Veículo'"
                        }
                        column_errors.append("control_board")
                else:
                    item_dict["control_board_error"] = {
                        "error": "Quadro de controle é obrigatório"
                    }
                    column_errors.append("control_board")
                ...
                self.preview_contract_items_administration.append(item_dict)

            return len(self.preview_contract_items_administration) > 0
    ...
    def get_preview_data(self):
        ...
        if self.preview_contract_items_administration:
            for item in self.preview_contract_items_administration:
                item_data = {
                    ...
                    "control_board": item.get("control_board", ""),
                }
                ...
                preview_data["contract_items_administration"].append(item_data)
        return preview_data
...
def generate_preview(excel_import_id, user_id):
    ...
    try:
        ...
        importer = ImportContractItemAdministration(excel_import, user)
...
def execute_import(excel_import_id):
    try:
        ...
        importer = ImportContractItemAdministration(
...
def create_contract_items_from_preview(importer, preview_data):
    ...
    if not permissions.has_permission(permission="can_create"):
        sentry_sdk.capture_exception(
            "kartado.error.contract_item_administration.user_does_not_have_permission_to_create"
        )
        return False
    ...
    for item in preview_data.get("contract_items_administration", []):
        ...
        try:
            ...
            if item.get("section_name"):
                section = ContractService.objects.filter(
                    ...
                    administration_service_contracts__uuid=contract_id,
                ).first()
            else:
                section = None
            ...
            last_order = ContractItemAdministration.objects.filter(
                contract_item_administration_services=section,
            ...
            if item.get("control_board"):
                control_board_clean = clean_latin_string(
                    item["control_board"].lower().strip()
                )
                ...
                if control_board_clean in control_board_mapping:
                    app_label, model = control_board_mapping[control_board_clean]
                    contract_item_data["content_type"] = ContentType.objects.get(
                        app_label=app_label, model=model
                    )
            ...
            contract_item = ContractItemAdministration.objects.create(
                **contract_item_data
            )

            if section:
                contract_item.contract_item_administration_services.add(section)

            contract_items.append(contract_item)

            excel_contract_item = {
                "contract_item_administration_id": str(contract_item.uuid),
                "excel_import_id": str(importer.excel_import.uuid),
                "row": str(item.get("row", 0)),
                "operation": "CREATE",
            }

            serialized_excel_contract_item = ExcelContractItemAdministrationSerializer(
                data=excel_contract_item
            )

            if serialized_excel_contract_item.is_valid():
                ExcelContractItemAdministration.objects.create(**excel_contract_item)

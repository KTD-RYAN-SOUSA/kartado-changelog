...
from helpers.strings import COMMON_IMAGE_TYPE, format_km, int_set_zero_prefix
...
class XlsxHandler:
    def __init__(
        ...
        self.__xlsx_file = (
            "./fixtures/reports/ccr_report_artesp_surface_drainage_sheets_v2025.xlsx"
        )
        ...
        self.static_fields_sheet = {
            ...
            "limpeza": "E82",
            "extensao_limpeza_ini": "M82",
            "extensao_limpeza_meio": "O82",
            "extensao_limpeza_fim": "Q82",
            "desobstrucao": "E84",
            "extensao_desobstrucao_ini": "M84",
            "extensao_desobstrucao_meio": "O84",
            "extensao_desobstrucao_fim": "Q84",
            "rocada": "E86",
            "extensao_rocada_ini": "M86",
            "extensao_rocada_meio": "O86",
            "extensao_rocada_fim": "Q86",
            "reparo": "E88",
            "extensao_reparo_ini": "M88",
            "extensao_reparo_meio": "O88",
            "extensao_reparo_fim": "Q88",
            "reconstrucao": "E90",
            "extensao_reconstrucao_ini": "M90",
            "extensao_reconstrucao_meio": "O90",
            "extensao_reconstrucao_fim": "Q90",
            "influencia_pavimento_ent": "V82",
            "erosao_ent": "V84",
            "observacoes_diagnostico": "E92",
        }
    ...
    def fill_sheet(self, data_list: list):
        ...
        for data in data_work:
            ...
            # Preencha Atuação
            atuation_value = data.get("atuacao", False)
            atuation_true_column = "E74"
            atuation_false_column = "E76"
            if atuation_value:
                self._worksheet[atuation_true_column] = "X"
            else:
                self._worksheet[atuation_false_column] = "X"
    ...
    def create_dict(self, reporting: Reporting, s3) -> dict:
        ...
        qs_reporting_files = (
            ReportingFile.objects.exclude(pk__in=pks_in_croqui)
        ...
        LIMPEZA = form_data_display.get("limpeza")
        EXTENSAO_LIMPEZA_INI = (
            form_data_display.get("extensao_limpeza_ini", "-") if LIMPEZA else "-"
        )
        EXTENSAO_LIMPEZA_MEIO = (
            form_data_display.get("extensao_limpeza_meio", "-") if LIMPEZA else "-"
        )
        EXTENSAO_LIMPEZA_FIM = (
            form_data_display.get("extensao_limpeza_fim", "-") if LIMPEZA else "-"
        )

        DESOBSTRUCAO = form_data_display.get("desobstrucao")
        EXTENSAO_DESOBSTRUCAO_INI = (
            form_data_display.get("extensao_desobstrucao_ini", "-")
            if DESOBSTRUCAO
            else "-"
        )
        EXTENSAO_DESOBSTRUCAO_MEIO = (
            form_data_display.get("extensao_desobstrucao_meio", "-")
            if DESOBSTRUCAO
            else "-"
        )
        EXTENSAO_DESOBSTRUCAO_FIM = (
            form_data_display.get("extensao_desobstrucao_fim", "-")
            if DESOBSTRUCAO
            else "-"
        )

        ROCADA = form_data_display.get("rocada")
        EXTENSAO_ROCADA_INI = (
            form_data_display.get("extensao_rocada_ini", "-") if ROCADA else "-"
        )
        EXTENSAO_ROCADA_MEIO = (
            form_data_display.get("extensao_rocada_meio", "-") if ROCADA else "-"
        )
        EXTENSAO_ROCADA_FIM = (
            form_data_display.get("extensao_rocada_fim", "-") if ROCADA else "-"
        )

        REPARO = form_data_display.get("reparo")
        EXTENSAO_REPARO_INI = (
            form_data_display.get("extensao_reparo_ini", "-") if REPARO else "-"
        )
        EXTENSAO_REPARO_MEIO = (
            form_data_display.get("extensao_reparo_meio", "-") if REPARO else "-"
        )
        EXTENSAO_REPARO_FIM = (
            form_data_display.get("extensao_reparo_fim", "-") if REPARO else "-"
        )

        RECONSTRUCAO = form_data_display.get("reconstrucao")
        EXTENSAO_RECONSTRUCAO_INI = (
            form_data_display.get("extensao_reconstrucao_ini", "-")
            if RECONSTRUCAO
            else "-"
        )
        EXTENSAO_RECONSTRUCAO_MEIO = (
            form_data_display.get("extensao_reconstrucao_meio", "-")
            if RECONSTRUCAO
            else "-"
        )
        EXTENSAO_RECONSTRUCAO_FIM = (
            form_data_display.get("extensao_reconstrucao_fim", "-")
            if RECONSTRUCAO
            else "-"
        )

        INFLUENCIA_PAVIMENTO_ENT = form_data_display.get("influencia_pavimento_ent")
        EROSAO_ENT = form_data_display.get("erosao_ent")
        OBSERVATION_DIAGNOSTIC = form_data_display.get("observacoes_diagnostico")
        ATUACAO = form_data_display.get("atuacao")
        ...
        data = {
            ...
            "limpeza": LIMPEZA,
            "extensao_limpeza_ini": EXTENSAO_LIMPEZA_INI,
            "extensao_limpeza_meio": EXTENSAO_LIMPEZA_MEIO,
            "extensao_limpeza_fim": EXTENSAO_LIMPEZA_FIM,
            "desobstrucao": DESOBSTRUCAO,
            "extensao_desobstrucao_ini": EXTENSAO_DESOBSTRUCAO_INI,
            "extensao_desobstrucao_meio": EXTENSAO_DESOBSTRUCAO_MEIO,
            "extensao_desobstrucao_fim": EXTENSAO_DESOBSTRUCAO_FIM,
            "rocada": ROCADA,
            "extensao_rocada_ini": EXTENSAO_ROCADA_INI,
            "extensao_rocada_meio": EXTENSAO_ROCADA_MEIO,
            "extensao_rocada_fim": EXTENSAO_ROCADA_FIM,
            "reparo": REPARO,
            "extensao_reparo_ini": EXTENSAO_REPARO_INI,
            "extensao_reparo_meio": EXTENSAO_REPARO_MEIO,
            "extensao_reparo_fim": EXTENSAO_REPARO_FIM,
            "reconstrucao": RECONSTRUCAO,
            "extensao_reconstrucao_ini": EXTENSAO_RECONSTRUCAO_INI,
            "extensao_reconstrucao_meio": EXTENSAO_RECONSTRUCAO_MEIO,
            "extensao_reconstrucao_fim": EXTENSAO_RECONSTRUCAO_FIM,
            "influencia_pavimento_ent": INFLUENCIA_PAVIMENTO_ENT,
            "erosao_ent": EROSAO_ENT,
            "observacoes_diagnostico": OBSERVATION_DIAGNOSTIC,
            "atuacao": ATUACAO,
            "photos": photos,
            "km_float": KM_FLOAT,
            "file_name": file_name,
...
class XlsxHandlerReportSurfaceDrainageSheetsv2025(CCRReport):
    def __init__(
        self, uuids: List[str] = None, report_format: ReportFormat = ReportFormat.XLSX
    ) -> None:
        self.class_name = "Monitoração de Drenagem Superficial (v2025)"
...
@task
def ccr_report_surface_drainage_async_handler_v2025(reporter_dict: dict):
    reporter = XlsxHandlerReportSurfaceDrainageSheetsv2025.from_dict(reporter_dict)
    reporter.export()

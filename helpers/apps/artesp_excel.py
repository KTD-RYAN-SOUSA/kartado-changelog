...
class ArtespExcelEndpoint:
    ...
    def __init__(
        ...
        schedule_data,
        excel_name,
        template_path="fixtures/reports/artesp_excel.xlsx",
    ):
        ...
        self.file_path_name = "{}/{}.xlsx".format(self.path_excel, self.excel_name)
    ...
    def set_print_area(self):
        ...
        end_row = self.page_count * 28 + 3

        ws.print_area = "A1:JB{}".format(end_row)
    ...
    def hide_date_columns(self):
        ...
        for year_index, year in enumerate(range(start_year, end_year + 1)):
            ws.cell(row=3, column=79 + year_index * 12).value = year
    ...
    def add_inventory_items(self):
        ...
        for index, inventory in enumerate(self.schedule_data["inventory"]):
            page_index = math.floor(index / 2)
            main_row = (
                4 + (page_index * self.page_offset) + (10 if index % 2 == 1 else 0)
            )
            ws.cell(row=main_row, column=1).value = index + 1
            ws.cell(row=main_row, column=2).value = inventory["number"]
            ws.cell(row=main_row, column=3).value = inventory["occurrence_type_name"]
            ws.cell(row=main_row, column=4).value = inventory["km"]
            ws.cell(row=main_row, column=5).value = inventory["direction"]
            ...
            if initial_inspection:
                ws.cell(row=main_row, column=6).value = datetime_to_date(
                    initial_inspection["date"]
                )
                ws.cell(row=main_row, column=8).value = initial_inspection[
                    "structural_classification"
                ]
                ws.cell(row=main_row, column=9).value = initial_inspection[
                    "functional_classification"
                ]
                ws.cell(row=main_row, column=10).value = initial_inspection[
                    "wear_classification"
                ]

            if special_inspection:
                ws.cell(row=main_row, column=72).value = special_inspection[
                    "structural_classification"
                ]
                ws.cell(row=main_row, column=73).value = special_inspection[
                    "functional_classification"
                ]
                ws.cell(row=main_row, column=74).value = special_inspection[
                    "wear_classification"
                ]
            ...
            for job_index in range(9):
                ...
                if job["start_date"]:
                    ws.cell(
                        row=main_row + 1 + job_index, column=77
                    ).value = datetime_to_date(job["start_date"])

                if job["end_date"]:
                    ws.cell(
                        row=main_row + 1 + job_index, column=78
                ...
                if job["start_date"] and job["end_date"]:
                    ...
                    start_column = 79 + year_offset * 12 + month_offset
                    end_column = start_column + len(date_list) - 1
                    row_style = ws.cell(row=main_row + 1 + job_index, column=76)._style
            ...
            ws.cell(row=main_row + 1, column=259).value = jobs["done"]
            ws.cell(row=main_row + 1, column=261).value = jobs["pending"]
    ...
    def get_data(self):
        ...
        for _ in range(math.floor(len(self.schedule_data["inventory"]) / 2)):
            self.add_page()
...
@task
def run_async_artesp_excel_export(uuids, company, excel_name):

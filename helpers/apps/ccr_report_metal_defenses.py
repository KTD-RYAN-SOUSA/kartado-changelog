...
from helpers.apps.ccr_resume_description_measures_metal_defenses import (
    XlsxHandlerResumeReportDescriptionMeasuresMetalDefenses,
)
from helpers.strings import clean_latin_string, format_km

from .ccr_resume_report_metal_defenses import XlsxHandlerResumeReportMetalDefenses


class MetalDefensesXlsxHandler(object):
    def __init__(
        self,
        s3,
        list_uuids: List[str],
        ...
        self.wb = load_workbook("./fixtures/reports/ccr_report_metal_defenses.xlsx")
        ...
        self.static_fields = {
            ...
            "conversion": {
                "FERRUGEM": "S",
                "PINTURA_DEFICIENTE": "S",
                "AMASSADOS": "S",
            },
            "reflectors": {"AUSENTE": "U", "RUIM": "U", "REGULAR": "U", "BOM": "U"},
            "reflectors_color": {
                "tipo_refletor": "W",
                "VERMELHO": "W",
                "BRANCO": "W",
    ...
    def create_dict(self, reporting):
        ...
        reflector_state = new_get_form_data(reporting, "reflectiveState")
        ...
        _reflectors_color = MetalDefensesXlsxHandler.__delineator_state(reporting)
        ...
        result_reflectors_color = {
            ...
            "VERMELHO": "X" if "Vermelho" in _reflectors_color else "",
            "BRANCO": "X" if "Branco" in _reflectors_color else "",
            "LIMA-LIMAO": "X" if "Lima-Limão" in _reflectors_color else "",
        }

        result_soil = new_get_form_data(reporting, "soil")
        result_screw = new_get_form_data(reporting, "screw")
        result_pointers = new_get_form_data(reporting, "pointers")

        _fixation = {
            "SOLO": "Sim" if result_soil else "Não",
            "PARAFUSOS": "Sim" if result_screw else "Não",
            "PONTEIRAS": "Sim" if result_pointers else "Não",
        }

        result_crumples = new_get_form_data(reporting, "crumples")

        result_deficient_paint = new_get_form_data(reporting, "defficientPaint")
    ...
    def __insert_status_values(self, key, cell, value):
        ...
        if key == "tipo_refletor":
            ...
            MetalDefensesXlsxHandler.__format_fonts(cell=self._worksheet[cell], size=10)

        MetalDefensesXlsxHandler.__format_fonts(
            cell=cell_status_string, size=10, horizontal="left"
        )

    def insert_img(self, image: str, cell: str):
        img = Image(image)
        self._worksheet.add_image(img, cell)
    ...
    def fill_sheet(self, *, data_list: list):
        ...
        date_list.sort()
        filtered_date_list = [date.strftime("%d/%m/%Y") for date in date_list if date]

        if len(filtered_date_list) == 0:
            date_text = ""
        else:
            date_text = f"De {filtered_date_list[0]} a {filtered_date_list[-1]}"
        ...
        insert_centered_value(
            worksheet=self._worksheet,
            value="Elementos de Proteção e Segurança - Ficha de Monitoração de Defensas Metálicas",
            cell="L3",
            horizontal="left",
        )
    ...
    def execute(self):
        ...
        file_name = "Ficha de Monitoração - {} - Defensas Metálicas".format(road_name)
...
class CCRMetalDefenses(CCRReport):
    def __init__(
    ...
    def get_file_name(self):
        ...
        file_name = "Relatórios ANTT de Defensas Metálicas - {} - {}".format(
    ...
    def export(self):
        ...
        files.append(
            XlsxHandlerResumeReportMetalDefenses(
                list_uuids=self.uuids,
                s3=s3,
                sheet_target=self.sheet_target(),
            ).execute()
        ...
        description_measures = XlsxHandlerResumeReportDescriptionMeasuresMetalDefenses(
            uuid=self.uuids[0],
            list_uuids=self.uuids,
            s3=s3,
            sheet_target=self.sheet_target(),
...
@task
def ccr_report_metal_defenses_async_handler(reporter_dict: dict):
    reporter = CCRMetalDefenses.from_dict(reporter_dict)
    reporter.export()

# flake8: noqa
import sentry_sdk
from sentry_sdk.integrations.django import DjangoIntegration

from ..staging import *

# Database
DATABASES = {"default": credentials.CCR_HOMOLOG_DATABASE_URL}
AWS_STORAGE_BUCKET_NAME = credentials.AWS_STORAGE_CCR_HOMOLOG_BUCKET_NAME
AWS_S3_CUSTOM_DOMAIN = "%s.s3.sa-east-1.amazonaws.com" % AWS_STORAGE_BUCKET_NAME
AWS_S3_REGION_NAME = "sa-east-1"
AWS_DEFAULT_REGION = "sa-east-1"
STATIC_URL = "https://%s/%s/" % (AWS_S3_CUSTOM_DOMAIN, AWS_STATIC_LOCATION)

EMAIL_HOST = credentials.MAILTRAP_CCR["HOST"]
EMAIL_PORT = credentials.MAILTRAP_CCR["PORT"]
EMAIL_HOST_USER = credentials.MAILTRAP_CCR["USERNAME"]
EMAIL_HOST_PASSWORD = credentials.MAILTRAP_CCR["PASSWORD"]
EMAIL_USE_SSL = credentials.MAILTRAP_CCR["USE_SSL"]
EMAIL_USE_TLS = credentials.MAILTRAP_CCR["USE_TLS"]

BACKEND_URL = "https://homolog-ccr.api.kartado.com.br"
FRONTEND_URL = "https://homolog-ccr.kartado.com.br"

ARCGIS_SYNC_ENABLE = False

# Sentry Start
sentry_sdk.init(
    dsn=credentials.SENTRY_DATA_SOURCE_NAME,
    environment="homolog-ccr",
    integrations=[DjangoIntegration()],
    sample_rate=SENTRY_SAMPLE_RATE,
    traces_sample_rate=SENTRY_TRACES_SAMPLE_RATE,
    send_default_pii=True,
)

# SAML2 AUTH
SAML2_AUTH = {
    "DEFAULT_NEXT_URL": "/admin",
    # Custom target redirect URL after the user get logged in.
    # Default to /admin if not set.
    # This setting will be overwritten if you have parameter ?next= specificed in the login URL.
    "CREATE_USER": False,  # Create a new Django user when a new user logs in. Defaults to True.
    "USE_JWT": True,
    # Set this to True if you are running a Single Page Application (SPA)
    # with Django Rest Framework (DRF), and are using JWT authentication to authorize client users
    "DENIED_MESSAGE": "Seu usuário não foi configurado para utilizar o Kartado",
    "METADATA_AUTO_CONF_URL": "https://kartado-public-assets.s3.amazonaws.com/KARTADO-HOMOLOG-CCR.xml",
    "ENTITY_ID": "KARTADO-HOMOLOG-CCR",
    "FRONTEND_URL": "https://homolog-ccr.kartado.com.br/#/login",
    "ASSERTION_URL": "https://homolog-ccr.api.kartado.com.br",
    "ATTRIBUTES_MAP": {
        "email": "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress",
        "username": "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress",
    },
}

# xmlsec binary for saml2 lib when running on lambda
if os.environ.get("XMLSEC_BINARY"):
    SAML2_AUTH["XMLSEC_BINARY"] = os.environ["XMLSEC_BINARY"]

KARTADO_REPORTS_URL = (
    "https://2s44h0214g.execute-api.sa-east-1.amazonaws.com/ccr-homolog"
)

# ECS Fargate Configuration
ECS_CLUSTER_NAME = "ecs-ccr-homolog-cluster"
ECS_TASK_FAMILY = "ecs-ccr-homolog-family"
ECS_SUBNETS = [
    "subnet-0855c9a93c5b52c54",
    "subnet-0c433d7afa36bb70a",
    "subnet-0372a8ca124f9e288",
]
ECS_SECURITY_GROUPS = ["sg-089b111da3bea8878", "sg-09034cd22c625ed07"]
AWS_DEFAULT_REGION_ECS = "sa-east-1"
